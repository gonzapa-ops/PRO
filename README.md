...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, sans-serif; text-transform: uppercase; }
        ...
        /* SELLO DE AGUA */
        #sello-agua {
            display: none; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-30deg);
            font-size: 80px; font-weight: 900; color: rgba(0,0,0,0.1); border: 10px solid rgba(0,0,0,0.1);
            padding: 20px 50px; text-transform: uppercase; z-index: 0; pointer-events: none;
        }
...
<div class="main-container">
    ...
    <div id="bar-edicion">⚠️ MODO EDICIÓN ACTIVO (Datos de Cliente Bloqueados)</div>
            ...
            <div class="totals-card">
                ...
                <div class="final-row">
                    <div style="text-align:right; width:100%;">
                        ...
                        <div class="util-label">UTILIDAD: <span id="txtUtilidad" class="util-value-text">$0</span></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function bloquearEditor(estado) {
        ...
        document.querySelectorAll('#tablaItems button').forEach(b => b.style.display = 'none');
        document.querySelectorAll('input, select, textarea').forEach(el => {
            el.readOnly = true;
    ...
    function abrirGestor(type) {
        ...
        if(type==='clientes') {
            ...
            getDB(DB_KEYS.CLI).forEach((c,i)=> {
                tbody.innerHTML+=`<tr><td>${c.rut}</td><td>${c.razon}</td><td>${c.contacto}</td><td><button class="btn-small" style="background:#F39C12" onclick="openForm('CLI',${i})">EDIT</button><button class="btn-small" style="background:#C0392B" onclick="delItem('${DB_KEYS.CLI}',${i},'clientes')">DEL</button></td></tr>`;
            });
